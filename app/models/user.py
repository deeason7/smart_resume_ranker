...
class User(db.Model):
    ...
    A user can have one of two roles: 'candidate' or 'recruiter'. This model stores authentication details and defines
    relationships to jobs (for recruiters) and resumes(for candidates)
    """
    __tablename__ = 'user'
    ...
    #Defines the 'one-to-many' link where one User (recruiter) can have many Jobs.
    jobs = db.relationship('Job', back_populates='uploader', lazy=True, cascade="all, delete-orphan")
    #Defines the 'one-to-many' link where one User (candidate) can have many Resumes.
